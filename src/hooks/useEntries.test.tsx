...
type Result = {
  ...
  status?: number
  /** What `count: 'exact'` returns: the rows that match, before any range. */
  count?: number
}
...
/**
 * What PostgREST actually does: a read is capped server-side whether or not it
 * asked for a window. An unpaged read of a longer log therefore comes back
 * *short* and looks exactly like a complete one.
 */
let serverCap = 1000
/** Reads succeed this many times, then report no network. For a loop that dies mid-way. */
let readsBeforeFailing: number | null = null

function builder(): Builder {
  let writing = false
  /** The `.range()` asked for, so the fake pages exactly as PostgREST would. */
  ...
  const self: Builder = {
    ...
    order: () => self,
    range: (from, to) => {
      calls.push({ op: 'range', arg: [from, to] })
      window = [from, to]
      return self
    },
    then: (resolve, reject) => {
      ...
      if (!writing && readsBeforeFailing !== null) {
        if (readsBeforeFailing > 0) readsBeforeFailing -= 1
        else {
          const down: Result = { data: null, error: { message: 'no network' }, status: 0 }
      ...
      const result: Result = writing
        ...
        : {
            data: (window === null
              ? rowsOnServer
              : rowsOnServer.slice(window[0], window[1] + 1)
            ).slice(0, serverCap),
            error: null,
            status: 200,
            count: rowsOnServer.length,
          }
...
/**
 * The read that feeds the export, the question corpus, "On this day" and the
 * launch that re-arms every reminder — and the only read `reconcile` lets
 * delete rows it did not return.
 */
describe('reading the whole log', () => {
  /** More than one page, so a read that does not page comes back short. */
  function longLog(count: number) {
    rowsOnServer = Array.from({ length: count }, (_, at) =>
      serverRow({
        id: `server-${at}`,
        occurred_on: '2026-09-05',
        created_at: `2026-09-05T09:00:${String(at % 60).padStart(2, '0')}+05:30`,
      }),
    )
  }

  it('keeps asking until it has as many rows as the server counted', async () => {
    longLog(2300)
    const { result } = await mounted()
  ...
  it('returns the whole log even though the server caps every response', async () => {
    // The bug: unpaged, this read came back with the first 1000 rows and no
    // indication it was short — and a full read is authoritative, so the 500
    // rows past the cap were deleted from this device on launch. The export,
    // the question corpus and "On this day" are all built on this call.
    longLog(1500)
    const { result } = await mounted()
  ...
  it('pages past a server cap smaller than the page it asks for', async () => {
    // The cap is a server setting. Below `PAGE` every page comes back short,
    // so "short means the end" would stop on the first one and hand the whole
    // truncation straight to `reconcile` as the truth.
    longLog(1200)
    serverCap = 300
    const { result } = await mounted()
  ...
  it('removes nothing when a page fails partway through the loop', async () => {
    longLog(2500)
    const { result } = await mounted()
    await act(() => result.current.fetchAll())
    expect(result.current.all).toHaveLength(2500)

    // The log is unchanged on the server; this device simply cannot finish
    // reading it. What arrived still folds in, and nothing is dropped on the
    // strength of a read that stopped halfway.
