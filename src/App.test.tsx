...
function builder() {
  let writing = false
  /** The window `fetchAll` asked for, so the fake pages as PostgREST would. */
  ...
  self['then'] = (resolve: (value: Result) => unknown, reject?: (reason: unknown) => unknown) => {
    ...
    const page = window === null ? rowsOnServer : rowsOnServer.slice(window[0], window[1] + 1)
    return Promise.resolve(
      writing
        ? { error: null }
        : // `count` is the whole log, before the range — which is how the read
          // knows whether it has finished.
          { data: page, error: null, count: rowsOnServer.length },
    ).then(resolve)
  }
  return self
}
...
describe('a reminder that could not be re-armed', () => {
  ...
  it('re-arms the alarm when the delete is undone', async () => {
    // Deleting cancels every alarm the entry owns; `restore` only rewrites the
    // log. So a 5pm reminder deleted at 4:55 and undone at 4:56 came back onto
    // the timeline still reading 5:00 pm, with nothing armed until the next
    // launch — the silent reminder this app calls its worst failure, produced
    // by the button whose whole job is undoing a mistake.
    rowsOnServer = [row]
    await open()
    await userEvent.click(await screen.findByText('dentist'))
    await userEvent.click(screen.getByRole('button', { name: 'Delete' }))
    ...
    expect(vi.mocked(rearm).mock.calls.at(-1)?.[0]).toMatchObject({ id: row.id })
  })
})
