...
/**
 * Whether `close` belongs to the sheet on top — the one you can actually see.
 *
 * Every dismiss gesture has to agree about this. Android back already did,
 * because `back()` reads the same stack; Escape did not, and listened on
 * `window` with no notion of depth — so one press inside the manual opened from
 * the You sheet closed *both*, and focus returned to a button that had just
 * unmounted with it.
 */
export function topmost(close: () => void): boolean {
  return open.length > 0 && open[open.length - 1] === close
}
