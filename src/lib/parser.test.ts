...
describe('a refund, which is money going the other way', () => {
  ...
  // A hyphen between digits is not a minus sign — and the digits after one are
  // not an amount either. `` matched between the hyphen and the 1, so the
  // first number this found in `covid-19 test 500` was **19**, and the 500 that
  // is plainly the price was left sitting in a mangled title.
  it('does not read a hyphenated number as money at all', () => {
    const r = p('covid-19 test 500')
    expect(r?.amountPaise).toBe(50000)
    expect(r?.title).toBe('covid-19 test')
  })

  it('leaves a bare time range exactly as it was', () => {
    // It did not, before: `9-6` became an expense of ₹9 titled "6 work" — the
    // syntax this parser documents itself as refusing, silently turned into a
    // structurally wrong row. Nothing numeric is read out of it now.
    const r = p('9-6 work')
    expect(r?.kind).toBe('note')
    expect(r?.amountPaise).toBeUndefined()
    expect(r?.title).toBe('9-6 work')
  })

  it('does not read a unit stuck to a number as money', () => {
    const r = p('2kg rice 300')
    expect(r?.amountPaise).toBe(30000)
    expect(r?.title).toBe('2kg rice')
  })

  it('reads past a hyphenated code to the amount beside it', () => {
    expect(p('flight 6E-233 4500')?.amountPaise).toBe(450000)
    expect(p('call 3-4 people 500')?.amountPaise).toBe(50000)
  })

  it('still reads a number that merely ends a sentence', () => {
    expect(p('Lunch, 350.')?.amountPaise).toBe(35000)
  })
...
describe('robustness', () => {
  ...
  // A date phrase the parser turned *down* is still a date phrase. Left in the
  // text for the bare-number branch to find, the 31 became ₹31 and the row an
  // expense titled "dentist feb" — an invented amount on a line about a dentist.
  it('does not read a refused date as money', () => {
    const impossible = ['31 feb dentist', '30 feb dentist', '31 apr trip']
    for (const line of impossible) {
      const r = p(line)
      expect(r?.kind).toBe('note')
      expect(r?.amountPaise).toBeUndefined()
  ...
  it('does not read the year of a refused date as money either', () => {
    const r = p('anniversary 29 feb 2026')
    expect(r?.kind).toBe('note')
    expect(r?.amountPaise).toBeUndefined()
    expect(r?.title).toBe('anniversary 29 feb 2026')
  })

  // 2026 is not a leap year, so `29 feb` with no year matched nothing at all
  // and the 29 was read as ₹29. A leap-day anniversary is a documented thing
  // this app keeps; it could only be typed one year in four.
  it('reads a bare 29 February as the soonest February that has one', () => {
    for (const line of ['anniversary 29 feb', 'feb 29 anniversary']) {
      const r = p(line)
      expect(r?.kind).toBe('event')
  ...
  it('still honours a leap day the writer dated explicitly', () => {
    expect(p('anniversary 29 feb 2028')?.occurredOn).toBe('2028-02-29')
  })

  it('leaves an ordinary written date on this year, past or not', () => {
    expect(p('dentist 3 sep')?.occurredOn).toBe('2026-09-03')
    expect(p('15 aug holiday')?.occurredOn).toBe('2026-08-15')
    expect(p('dec 25 party 2000')?.occurredOn).toBe('2026-12-25')
  })
...
describe('a reminder that comes round again', () => {
  ...
  // `every weekdays` matched the bare `weekdays` matcher first, which takes the
  // word and leaves the `every` behind — so the row saved as "standup every".
  it('does not leave "every" in the title of an every-weekdays repeat', () => {
    const r = on('standup 10am every weekdays')
    expect(r?.data.rrule).toBe('FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR')
    expect(r?.title).toBe('standup')
  })
