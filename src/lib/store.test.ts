...
describe('folding a read into what this device holds', () => {
  ...
  // The same read, from a page loop that gave up. It has *seen* rows and can
  // say nothing about the ones it never reached — and an unpaged read of a long
  // log is exactly that, which is how launch came to delete every row past
  // PostgREST's cap from this device.
  it('drops nothing when the read did not cover everything it claims to', () => {
    const state = reconcile(
      local([
        entry({ id: 'beyond-the-cap', occurred_on: '2020-01-01' }),
        entry({ id: 'also-beyond', occurred_on: '2020-01-02' }),
      ]),
      [entry({ id: 'in-the-first-page', occurred_on: '2026-09-05' })],
      null,
      undefined,
      false,
    )
    expect(state.entries.map((row) => row.id).sort()).toEqual([
      'also-beyond',
      'beyond-the-cap',
      'in-the-first-page',
    ])
  ...
  it('still takes a partial read for what it does say', () => {
    // A row it *did* return replaces the copy held here — folding in is the
    // whole point of the read; only removal is withheld.
    const state = reconcile(
      local([entry({ id: 'known', title: 'old' })]),
      [entry({ id: 'known', title: 'new' })],
      null,
      undefined,
      false,
    )
    expect(state.entries).toHaveLength(1)
    expect(state.entries[0]?.title).toBe('new')
  })
