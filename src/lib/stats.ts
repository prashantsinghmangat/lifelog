 ...
 * totals line already follows, stated in ARCHITECTURE.md. The cost is agreed: the
...
function aggregate(rows: Entry[]): Totals {
  ...
  // Ranked by size, because size is what the track draws. A refund keeps its
  // sign in the figure — that rule is settled — but sorting by the signed value
  // while drawing by its magnitude put the *longest* bar at the bottom of the
  // list: a category that net-refunded ₹500 ranked last and filled the track,
  // while the category it outweighed sat at the top with a bar of nothing.
