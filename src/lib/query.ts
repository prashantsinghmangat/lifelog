...
const NOISE = new Set([
  ...
  // `measureOf` advertises "how long" as an hours question, and without `long`
  // here the word survived into the search terms and was matched against every
  // title — so "how long did i work this week" reliably found nothing, while
  // "hours worked this week" answered it.
  'long',
])
...
export function parseQuestion(input: string, now: Date): Question | null {
  ...
  // The period comes out first, and the measure is read from what is left.
  //
  // Measured against the raw text, the bare `days?` in `measureOf` matched
  // the *period* — so "how much did i spend in the last 3 days" was answered
  // with "3 days" as the headline, and the ₹690 it was asking for survived only
  // as a small extra underneath. "what happened 3 days ago" was a day count for
  // the same reason. A window is not a quantity the question is about.
  const { range: period, rest } = periodOf(asked, now)
  const measure = measureOf(rest)
