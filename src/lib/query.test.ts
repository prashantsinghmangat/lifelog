...
/**
 * The lead is the `text-3xl` headline and the first thing the live region
 * announces, so a measure read off the wrong part of the question is not a
 * detail — it is the answer.
 */
describe('reading the measure past the period', () => {
  const of = (text: string) => {
    const question = parseQuestion(text, NOW)
    if (question === null) throw new Error('not a question')
    return { question, said: answer(summarise(LOG, question, NOW), question, NOW) }
  }

  it('does not mistake a window of days for a question about days', () => {
    // `measureOf` ran on the raw text, where the bare `days?` matched the
    // *period*. "how much did i spend in the last 3 days" was answered with
    // "3 days" as the headline, and the money it asked for was demoted to a
    // small extra underneath it.
    expect(of('? how much did i spend in the last 3 days').question.measure).toBe('money')
    expect(of('? total spent last 30 days').question.measure).toBe('money')
    expect(of('? how many hours did i work in the last 3 days').question.measure).toBe('hours')
  })

  it('still reads a question that is genuinely about days', () => {
    expect(of('? how many days gym').question.measure).toBe('days')
    expect(of('? how many days did i go to the gym last week').question.measure).toBe('days')
  })

  it('answers "what happened N days ago" with the day, not with a count of days', () => {
    expect(of('? what happened 3 days ago').question.measure).toBeNull()
  })

  it('answers "how long", which it advertises and could never do', () => {
    // `long` was not a noise word, so it survived into the search terms and was
    // matched against every title — "how long did i work this week" reliably
    // found nothing, while "hours worked this week" answered it.
    const { question, said } = of('? how long did i work this week')
    expect(question.measure).toBe('hours')
    expect(question.terms).toEqual([])
    expect(said.lead).not.toBe('nothing found')
  })
})
