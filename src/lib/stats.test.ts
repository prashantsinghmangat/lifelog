...
describe('ranking the categories', () => {
  it('puts the longest track at the top, refund or not', () => {
    // The bar draws a category's *size* — a negative must never draw a negative
    // pixel, which is settled. Ranking by the signed value while drawing by the
    // magnitude meant a category that net-refunded ₹500 sorted last and filled
    // the track, under a category with the shortest bar in the list.
    const totals = totalsFor(
      [
        row({ kind: 'expense', occurred_on: '2026-09-14', amount_paise: 100, category: 'tea' }),
        row({ kind: 'expense', occurred_on: '2026-09-14', amount_paise: -50000, category: 'refunds' }),
        row({ kind: 'expense', occurred_on: '2026-09-14', amount_paise: 20000, category: 'food' }),
      ],
