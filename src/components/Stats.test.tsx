...
const ROWS: Entry[] = [
  ...
  // Late enough to fall outside the 6am–10pm bars while plainly having a time.
  row({
    kind: 'note',
...
describe('drilling', () => {
  ...
  it('does not tell a timed entry it has no time', async () => {
    // `leftOut` is the day's total minus what the bars hold, so it counts an
    // entry whose clock falls outside 6am–10pm as well as one with no clock at
    // all. Saying "has no time" of an 11:30pm note is a false statement in the
    // one line whose job is explaining why the bars and the total differ.
    open()
    await userEvent.click(screen.getByRole('button', { name: /^16 September/ }))
    expect(heading()).toBe('Wed 16 Sep')

    const hints = screen.getAllByRole('status').map((line) => line.textContent ?? '')
    expect(hints.some((line) => line.includes('not in the bars'))).toBe(true)
    expect(hints.some((line) => line.includes('no time and'))).toBe(false)
  })

  it('says how many entries stand in no hourly bar', async () => {
    open()
    await userEvent.click(screen.getByRole('button', { name: /^14 September/ }))
    // One of the 14th's two entries has no clock. The line names both
    // reasons a row can be missing, because `leftOut` counts both: an entry
    // logged at 11:30pm has a time and is still not in a bar, and telling it
    // it has none is a false claim in the line that exists to explain the gap.
    expect(
      screen.getByText('1 entry is not in the bars — no time, or outside 6am–10pm'),
    ).toBeTruthy()
  })
})
