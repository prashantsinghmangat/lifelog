...
/**
 * What to say when the auth call did not merely refuse but fell over.
 *
 * Every route on this screen clears `busy` in a `finally` for the same reason:
 * a throw that skipped it left the form disabled with nothing written on it.
 */
function said(failure: unknown): string {
  return failure instanceof Error ? failure.message : 'Could not reach the sign-in service'
}
...
export function Login({ onGuest, onCancel }: Props) {
  ...
  async function withPassword(event: FormEvent) {
    ...
    try {
      ...
      // supabase-js answers most failures with `{ error }`, but it rethrows
      // anything it does not recognise as an auth error. Uncaught, `setBusy`
      // never ran again and every control on this screen stayed disabled —
      // a sign-in form with no way forward and no way to find out why, which
      // is the one dead end the only screen into the app must not have.
      setMessage(said(failure))
    } finally {
      setBusy(false)
    }
  }
  ...
  async function request() {
    ...
    try {
      ...
      options: {
        ...
        /**
         * This is a single-user log, and without this line the sign-in box was
         * an open account-creation form.
         *
         * `signInWithOtp` creates the user when the address is unknown —
         * `shouldCreateUser` defaults to **true** — so anyone who reached this
         * screen and typed any address of their own became a real user of the
         * project, with their own rows, their own storage and their own place
         * in the auth table. Nothing in the UI offered to sign up; the default
         * did it for us.
         *
         * False means an unknown address is refused and a known one still gets
         * its link, so the owner's own sign-in is unchanged.
         *
         * This closes the *client*. It is not the whole control and must not be
         * read as one: anything can call the Supabase auth endpoint directly
         * with the publishable key, which ships in the bundle. Turning signups
         * off in the project's own settings is the layer that actually enforces
         * this — see README. This is the half that lives in the repository.
         */
