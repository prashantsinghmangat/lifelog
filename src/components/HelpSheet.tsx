            ...
            {section.examples.map((example) => (
              <li key={example.text}>
                <button
                  ...
                  // trying it are meant to be the same action.
                  className="flex min-h-11 w-full items-baseline gap-3 rounded-md px-2 py-2 text-left active:bg-sunken"
                >
                  {/* Not `shrink-0`: the longest example fits at the default
                      size and cannot yield at all under Android's 2x font
                      scaling, where it pushes the row past a sheet that scrolls
                      only vertically. */}
                  <code className="min-w-0 text-sm text-ink">{example.text}</code>
