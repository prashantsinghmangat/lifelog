...
**Create the one account in the dashboard, not through the app.** Authentication → Users → Add
user, with your email and a password. The app deliberately cannot create an account: `Login`
passes `shouldCreateUser: false`, because without it the sign-in box was an open sign-up form —
`signInWithOtp` creates the user whenever the address is unknown, and that default is on.

Then switch off **Allow new users to sign up** under Authentication → Sign In / Providers.

Both halves are needed and they are not the same control. The client is one layer; the publishable
key ships inside the bundle, so anything at all can call `/auth/v1/signup` directly and the only
thing that refuses it is the project setting. RLS is a third and separate thing: it keeps a second
user's rows away from yours, and it does not stop the second user existing.

You can check the setting rather than remember it:
